(function initDublinNotesLogic(root) {
  ...
  function exportFeedbackText(state, labels = {}) {
    ...
    return ['Dublin notes', ...sections].join('\n\n');
  }
  ...
  function exportSharePacket(state) {
    return `DUBLIN_TRIP_NOTES_V1\n${JSON.stringify(buildSubmissionPacket(state), null, 2)}`;
  }

  function parseSharePacket(text) {
    const raw = String(text || '').trim();
    const json = raw.startsWith('DUBLIN_TRIP_NOTES_V1')
      ? raw.replace(/^DUBLIN_TRIP_NOTES_V1\s*/, '')
      : raw;
  ...
  function getCountdownItems(now = new Date()) {
    ...
    const items = [
      ['trip', 'Dublin begins', '2026-07-03T00:00:00-07:00'],
      ['mister-s', 'Mister S prime table', '2026-06-03T00:00:00-07:00'],
      ['delahunt', 'Delahunt / Library Street backup lock', '2026-06-03T00:00:00-07:00'],
      ['uno-mas', 'Uno Mas 60-day window', '2026-05-04T00:00:00-07:00'],
      ['week-of', 'Week-of music/listings check', '2026-06-26T00:00:00-07:00'],
      ['airport', 'Departure buffer check', '2026-07-05T00:00:00-07:00']
    ];
