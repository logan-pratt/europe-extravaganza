...
test('buildSubmissionPacket returns a plain object with version, exportedAt, and state', () => {
  const state = saveOptionFeedback(createEmptyNotesState(), 'pub:mulligan', 'Logan', {
    reaction: 'love',
    note: 'Classic Dublin.'
  });
...
test('exportSharePacket text contains the same data as buildSubmissionPacket', () => {
  const state = saveOptionFeedback(createEmptyNotesState(), 'pub:mulligan', 'Emily', {
    reaction: 'love',
    note: ''
  });
  ...
  assert.ok(text.startsWith('DUBLIN_TRIP_NOTES_V1'));
  const parsed = JSON.parse(text.replace(/^DUBLIN_TRIP_NOTES_V1\s*/, ''));
